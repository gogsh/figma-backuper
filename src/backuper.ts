...
class Backuper {
  ...
  async backupOneUser(user: User) {
    if (this.options.verbose)
      console.log(`Backup for user ${user.login} started`);
  ...
  async backupProjectsAndTeams(user: User) {
    if (this.options.verbose)
      console.log('Backup all projects and teams, start');
    ...
    try {
      ...
      if (this.options.debug || this.options.verbose)
        console.log(`${user.login} Ошибка получения списка файлов!`);
      if (this.options.debug) console.log(e);

      this.currentReportData.errors.push(
        `${user.login} Ошибка получения списка файлов!`,
      );
    ...
    for (let i = 0; i < figmaApiLinks.length; i++) {
      let projectFolder = fsHelper.prepareFolderName(
        userFolder,
        figmaApiLinks[i].team.name,
        figmaApiLinks[i].project.name,
      );
    ...
    if (this.options.verbose)
      console.log('Backup all projects and teams, done');
  }
  ...
  async backupRecent(user: User) {
    ...
    if (this.options.debug)
      console.log('    waitForElementAndGet', selector.recentFilesSelector);
    const recentRows = await this.waitForElementAndGet(
      selector.recentFilesSelector,
      true,
    );

    const regexpTitle =
      /<div[^>]+class[^>]+generic_tile--title[^>]+>([^<]+)<\/div>/;
    const regexpTime = /Edited[^<]*<span[^>]*>([^<]+)<\/span>/;
    const regexpTimeParts =
      /(?<num>\d+|last)\s(?<type>minute|hour|day|month|year)s?(\sago)?/;
    ...
    for (let i = 0; i < recentRows.length; i++) {
      ...
      if (this.options.all) {
        ...
        if (matchTime === 'yesterday') {
          ...
          if (this.options.debug)
            console.log('Time: ' + matchTime, 'AS: ', timeParts);
          minutesSinceUpdate =
            (timeKoeffs[timeParts.groups.type]
              ? timeKoeffs[timeParts.groups.type]
              : 1) *
            parseInt(
              timeParts.groups.num === 'last' ? 1 : timeParts.groups.num,
            );
        }
    ...
    for (let currentTry = 0; currentTry < this.MAX_TRIES; currentTry++) {
      ...
      for (let i = 0; i < currentArray.length; i++) {
        ...
        if (this.options.verbose) {
          ...
          this.currentReportData.statistics.push(
            (result ? 'Скачал ' : 'НЕ СКАЧАЛ ') + title + ' за ' + timeForOne,
          );
        }
      ...
      if (newArray.length === 0) {
        ...
        if (this.options.verbose) {
          this.currentReportData.statistics.push(
            '===== Следующая итерация попыток скачивания =====',
          );
        }
      }
    }

    this.currentReportData.filesSaved =
      linksToFolders.length - currentArray.length;
    if (currentArray.length > 0) {
      for (let i = 0; i < currentArray.length; i++) {
        this.currentReportData.errors.push(
          "Can't download " + currentArray[i].link,
        );
      }
    }
  }
  ...
  async sendReport(reports: Report[]) {
    ...
    let partlyHTML =
      '<h1 style="color:yellow">#USER#: ОК, но с предупреждениями</h1>';
    ...
    for (let i = 0; i < reports.length; i++) {
      ...
      const statistics = this.options.verbose
        ? report.statistics.join('<br>')
        : '';

      if (!report.filesShouldBe) {
        const hours = this.hoursForPartialBackup;
        const text = this.options.all
          ? 'Списов файлов пуст!'
          : `За последние ${hours}ч нет изменённых файлов.`;
        fullReport.push(`<h1>${report.login}: ${text}</h1>`);
      } else {
        const header = (
          isDownloadedAll ? (hasErrors ? partlyHTML : goodHTML) : badHTML
  ...
  async getUserLinks(user: User): Promise<LinksToProjectsAndTeams[]> {
    return await api.createLinksToFiles(
      user.teams,
      user.token,
      this.options.all,
      this.hoursForPartialBackup,
    );
  }
  ...
  async downloadOneFile(linkToFolder: LinkToFolder): Promise<Boolean> {
    if (this.options.verbose)
      console.log('downloadOneFile start', linkToFolder.link);
    ...
    if (this.options.debug)
      console.log('    driver.get(link)', linkToFolder.link);
    ...
    try {
      ...
      if (this.options.debug)
        console.log(
          '    waitForElementAndGet done ' + timeForOne,
      ...
      this.currentReportData.errors.push(
        `exception ${selector.folderNameInFile}, ${linkToFolder.link}, ${timeForOne}`,
      );
  ...
  async saveToDisk(
    ...
    try {
      ...
      const input = await this.webdriver.findElement(
        WebDriver.By.css(selector.quickActionsInput),
      );
      ...
      if (this.options.debug)
        console.log('Send "Save local" command, waiting for file');

      // чтобы файл успел скачаться
      let count = Math.round(this.delayFileDownload / this.period);
      const success = await this.waitExistenceOfFile(
        count,
        title,
        folder,
        link,
      ...
      this.currentReportData.errors.push(
        `StaleElementReferenceException "html"`,
      );
      // неуспешный результат, в отчете будет показан как "есть ошибки"
      return false;
    }
  ...
  async login(user: User, driver) {
    ...
    // дождаться загрузки страницы
    if (this.options.debug)
      console.log('    waitForElementAndGet', selector.authBlock);
    await this.waitForElementAndGet(selector.authBlock);
    if (this.options.debug)
      console.log('    waitForElementAndGet done', selector.authBlock);

    const emailInput = await this.waitForElementAndGet(selector.authFieldLogin);
    const passwordInput = await this.waitForElementAndGet(
      selector.authFieldPassword,
    );
    const loginButton = await this.waitForElementAndGet(selector.loginLink);

    if (this.options.debug)
      console.log('    waitForElementAndGet (email, pass, btn) done');
    ...
    try {
      ...
      if (this.options.debug) {
        console.log(
          '    waitForElementAndGet error ' + selector.menuLinkDrafts,
        );
        console.log(exception);
  ...
  async getWebdriver(user: null | User = null, force = false) {
    ...
    this.webdriver = this.createSessionNewDriver(
      fsHelper.prepareFolderName(this.baseFolder, 'temp'),
    );

    if (this.options.debug)
      console.log(
        '    WebDriver autosave folder: ' +
          fsHelper.prepareFolderName(this.baseFolder, 'temp'),
      );
