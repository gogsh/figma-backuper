...
const getProjectsByTeam = async (
  ...
  for (let i = 0; i < MAX_TRIES; i++) {
    try {
      const response = await axios.get(`${urlApi}teams/${team.id}/projects`, {
...
const getProjectFiles = async (projectId: string, token: string) => {
  ...
  for (let i = 0; i < MAX_TRIES; i++) {
    try {
      const response = await axios.get(`${urlApi}projects/${projectId}/files`, {
        headers: { 'X-FIGMA-TOKEN': token },
      });
 ...
 */
const filesInProjectWorthVisiting = async (
  projectId: string,
  token: string,
  getAllFiles = false,
  hoursToGetOld = 0,
...
const getFilesToVisit = async (
  ...
  for (let i = 0; i < projectIds.length; i++) {
    const newFiles = await filesInProjectWorthVisiting(
      projectIds[i],
      token,
      getAllFiles,
      hoursToGetOld,
...
const getFilesByTeams = async (
  ...
  const filesLinkedToProjects = await getFilesToVisit(
    projectIds,
    token,
    getAllFiles,
    hoursToGetOld,
 ...
 */
const linkFilesAndTeams = (
  projectsTeams: ProjectToTeams[],
  filesToProjects: FilesToProjects[],
) => {
  ...
  for (let i = 0; i < filesToProjects.length; i++) {
    for (let j = 0; j < projectsTeams.length; j++) {
      if (
        projectsTeams[j].project.id.toString() === filesToProjects[i].projectId
      ) {
 ...
 */
async function createLinksToFiles(
  teams: { id: string; name: string }[],
  token: string,
  getAllFiles = false,
  hoursToGetOld = 0,
): Promise<LinksToProjectsAndTeams[]> {
  const filesToProjectsAndTeams = await getFilesByTeams(
    teams,
    token,
    getAllFiles,
    hoursToGetOld,
